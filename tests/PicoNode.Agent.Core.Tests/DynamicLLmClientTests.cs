...
public class DynamicLLmClientTests
{
    ...
    public async Task UnconfiguredAgent_ReturnsErrorText()
    {
        ...
        await foreach (
            var e in client.StreamAsync(new PicoNode.AI.Model(), ctx, null, CancellationToken.None)
        )
            events.Add(e);
    ...
    public async Task RunTurnWithDynamicClient_ShowsErrorToUser()
    {
        ...
        await Assert
            .That(events.Any(e => e.Item1 == "text" && e.Item2?.Contains("No API key") == true))
            .IsTrue();
    }
    ...
    private static PicoNode.Agent.Domain.Agent CreateAgent(string provider, string model)
    {
        ...
        return new PicoNode.Agent.Domain.Agent(
            Guid.CreateVersion7(),
            llms,
            provider,
            model,
