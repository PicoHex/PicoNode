...
public sealed class AgentInvariantTests
{
    ...
    {
        var (system, _) = Create();
        await Assert.That(async () => await system.CreateAsync<DomainAgent>(new CreateAgent([], "a", "b", "/tmp"))).Throws<DomainInvariantException>();
    }
    ...
    {
        var (system, _) = Create();
        await Assert.That(async () => await system.CreateAsync<DomainAgent>(new CreateAgent([new() { ApiKey = "" }], "a", "a", "/tmp"))).Throws<DomainInvariantException>();
    }
    ...
    {
        var (system, _) = Create();
        var a = await system.CreateAsync<DomainAgent>(new CreateAgent(DefaultLlms(), "deepseek", "chat", "/tmp"));
        system.Send(a.Id, new StartAgent()); await Task.Delay(100);
    ...
    public async Task Complete_Transitions()
    {
        var (system, _) = Create();
        var a = await system.CreateAsync<DomainAgent>(new CreateAgent(DefaultLlms(), "deepseek", "chat", "/tmp"));
        system.Send(a.Id, new StartAgent()); await Task.Delay(50);
        system.Send(a.Id, new CompleteAgent()); await Task.Delay(100);
    ...
    public async Task RemoveLlm_OnlyOne_Throws()
    {
        var (system, _) = Create();
        var a = await system.CreateAsync<DomainAgent>(new CreateAgent(DefaultLlms(), "deepseek", "chat", "/tmp"));
        system.Send(a.Id, new RemoveLlmCmd("deepseek", "chat")); await Task.Delay(100);
    ...
    public async Task AddTool_Duplicate_Throws()
    {
        var (system, _) = Create();
        var a = await system.CreateAsync<DomainAgent>(new CreateAgent(DefaultLlms(), "deepseek", "chat", "/tmp"));
        system.Send(a.Id, new AddToolCmd(new Tool { Name = "read" })); await Task.Delay(50);
        system.Send(a.Id, new AddToolCmd(new Tool { Name = "read" })); await Task.Delay(100);
