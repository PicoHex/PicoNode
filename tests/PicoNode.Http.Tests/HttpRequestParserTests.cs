...
public sealed class HttpRequestParserTests
{
    ...
    public async Task Parse_preserves_repeated_request_headers_and_combines_lookup_values()
    {
        ...
        await Assert.That(request.HeaderFields[0]).IsEqualTo(new KeyValuePair<string, string>("Host", "example.com"));
        await Assert.That(request.HeaderFields[1]).IsEqualTo(new KeyValuePair<string, string>("Accept", "text/plain"));
        await Assert.That(request.HeaderFields[2]).IsEqualTo(new KeyValuePair<string, string>("accept", "text/html"));
    ...
    {
        var buffer = CreateSequence(Encoding.ASCII.GetBytes("GET foo HTTP/1.1\r\nHost: example.com\r\n\r\n"));
    ...
    {
        var buffer = CreateSequence(Encoding.ASCII.GetBytes("GET /hello#frag HTTP/1.1\r\nHost: example.com\r\n\r\n"));
    ...
    {
        var buffer = CreateSequence(Encoding.ASCII.GetBytes("GET /%GG HTTP/1.1\r\nHost: example.com\r\n\r\n"));
    ...
    {
        var buffer = CreateSequence(Encoding.ASCII.GetBytes("GET /hello%20world?name=pico HTTP/1.1\r\nHost: example.com\r\n\r\n"));
