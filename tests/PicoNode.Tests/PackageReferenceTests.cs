...
public sealed class PackageReferenceTests
{
    ...
    [Test]
    public async Task PicoNode_csproj_has_PicoCfg_Abs()
    {
        // TcpNodeOptions.Config exposes ICfgRoot: the transport package is where
        // PicoCfg.Abs is genuinely used and therefore declared.
        var refs = GetPicoHexPackageRefs("src/Network/PicoNode/PicoNode.csproj");
    ...
    [Test]
    public async Task PicoNode_Http_csproj_has_no_redundant_PicoCfg_Abs()
    {
        // The HTTP layer neither uses ICfg in code nor exposes it in its public
        // API; the direct reference was redundant and was pruned.
        var refs = GetPicoHexPackageRefs("src/Http/PicoNode.Http/PicoNode.Http.csproj");
        await Assert.That(refs).DoesNotContain("PicoCfg.Abs");
    }
