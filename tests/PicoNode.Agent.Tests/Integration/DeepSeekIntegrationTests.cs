...
public class DeepSeekIntegrationTests
{
    ...
    public async Task DeepSeekChat_Streaming_ReturnsTextResponse()
    {
        ...
        {
            Messages = [new Message { Role = "user", Content = "Say hello in exactly 3 words.", Timestamp = 1 }],
        };
    ...
    {
        var apiKey = Environment.GetEnvironmentVariable("DEEPSEEK_API_KEY") ?? "";
        if (string.IsNullOrEmpty(apiKey)) { Console.WriteLine("DEEPSEEK_API_KEY not set, skipping"); return; }
        ...
        var model = new Model
        {
            Id = "deepseek-chat", BaseUrl = "https://api.deepseek.com/v1",
            Api = AiApiFormat.OpenAIChatCompletions, Provider = "deepseek", MaxTokens = 256,
        };
        var loop = new AgentLoop(llmClient, registry, runner, model);

        var messages = new List<Message> { new() { Role = "user", Content = "Say hi", Timestamp = 1 } };
    ...
    public async Task ResilientClient_WithDeepSeek_RoutesAndReturns()
    {
        ...
        var http = new HttpClient { Timeout = TimeSpan.FromSeconds(30) };
        var clients = new Dictionary<string, ILLmClient> { ["deepseek"] = new OpenAILlmClient(http) };
        var router = new ProviderRouter(new[] { new ProviderConfig { Name = "deepseek", BaseUrl = "https://api.deepseek.com/v1", ApiFormat = AiApiFormat.OpenAIChatCompletions, Priority = 1 } });
        var breakers = new Dictionary<string, ICircuitBreaker>();
        var resilient = new ResilientLLmClient(router, breakers, clients);

        var model = new Model
        {
            Id = "deepseek-chat", Api = AiApiFormat.OpenAIChatCompletions, MaxTokens = 256,
        };
        var context = new ChatContext
        {
            Messages = [new Message { Role = "user", Content = "Say hi", Timestamp = 1 }],
        };
        var options = new StreamOptions { ApiKey = apiKey, MaxTokens = 256 };

        var events = new List<AssistantMessageEvent>();
        await foreach (var e in resilient.StreamAsync(model, context, options, CancellationToken.None))
            events.Add(e);
