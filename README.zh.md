...
[English](README.md) | **简体中文** | [繁體中文](README.zh-TW.md) | [Deutsch](README.de.md) | [Español](README.es.md) | [Français](README.fr.md) | [日本語](README.ja.md) | [한국어](README.ko.md) | [Português (Brasil)](README.pt-BR.md) | [Русский](README.ru.md)
...
|---------|----------|-------------|
| **依赖模型** | 无 Microsoft 框架依赖（仅 PicoHex 原生库）；分层按需选用 | `Microsoft.AspNetCore.App` 框架引用 |
...
| **代码行数** | 全栈约 18K 行 | ASP.NET Core 约 100 万+ 行 |
...
```
PicoNode.Abs        核心接口                                  (net10.0, zero deps)
    ↓
...
Controllers.Gen 源生成器：
- 扫描 `Controllers/` 文件夹（或 `[ApiController]` 类）
- 生成从 DI 解析控制器并完成注册的端点存根
...
|---------|--------|-------------|
| **PicoNode.Abs** | net10.0 | 核心接口：`INode`、`ITcpConnectionHandler`、`IUdpDatagramHandler`、故障码、枚举 |
