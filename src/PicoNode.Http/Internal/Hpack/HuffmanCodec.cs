...
/// <summary>
/// Shared Huffman encoding/decoding support per RFC 7541 Appendix B.
/// Provides the forward encode table (byte → code + bit-length) and the
/// binary decode tree used by <see cref="HpackDecoder"/>.
/// </summary>
internal static class HuffmanCodec
{
    // 257-entry table: symbol 0-255 + EOS (256). Tuple = (code right-justified, bit-length).
    private static readonly (uint Code, int BitLength)[] Codes = new (uint, int)[257];
    ...
    /// <summary>Huffman-encodes data. Returns encoded bytes.</summary>
    public static byte[] Encode(ReadOnlySpan<byte> data)
    {
        ...
        if (bitsInBuffer > 0)
            result[pos] = (byte)((bitBuffer << (8 - bitsInBuffer)) | ((0xFF >> bitsInBuffer) & 0xFF));

        return result;
    }

    /// <summary>Huffman-decodes data. Used by HpackDecoder.</summary>
