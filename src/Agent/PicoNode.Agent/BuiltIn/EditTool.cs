...
public sealed class EditTool : IBuiltInTool
{
    ...
    public string Description => "Edit files via precise text replacement. Each edit replaces oldText with newText. oldText must be unique and edits must not overlap.";
        ...
        CancellationToken ct)
    {
        ...
        if (!args.TryGetValue("edits", out var editsObj) || editsObj is not List<object?> editList || editList.Count == 0)
        ...
        for (int i = 0; i < editList.Count; i++)
        {
            if (editList[i] is not Dictionary<string, object?> edit)
                return Task.FromResult(($"[Error: edits[{i}] must be an object with oldText and newText]", true));
        ...
        for (int i = 0; i < edits.Count; i++)
        {
            ...
            if (idx < 0)
                return Task.FromResult(($"[Error: oldText not found: \"{Truncate(oldText, 60)}\"]", true));
            if (content.IndexOf(oldText, idx + 1, StringComparison.Ordinal) >= 0)
                return Task.FromResult(($"[Error: oldText appears multiple times: \"{Truncate(oldText, 60)}\"]", true));
        ...
        for (int i = 1; i < positions.Count; i++)
        {
            if (positions[i].Start < positions[i - 1].End)
                return Task.FromResult(("[Error: edits overlap. Each oldText must be in a non-overlapping region.]", true));
        }
