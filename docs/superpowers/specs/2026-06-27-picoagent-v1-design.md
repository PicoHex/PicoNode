...
`LocalEndPoint` 在 `ListenAsync` 后可用，port=0 时用于获取系统分配的实际端口。
...
- 错误统一格式 `{"type":"error","code":"...","message":"..."}`
  错误码: `EMPTY_INPUT`, `NOT_FOUND`, `CONFLICT`, `INVALID_ARGUMENT`, `INVALID_SESSION_ID`, `PROVIDER_UNAVAILABLE`, `NO_HOME_DIR`
- SSE 连接断开 → 对应的 LLM 调用自动取消
- `Agent.CreateAsync` 未传 `homeDir` → 会话保存/恢复端点返回 400
- sessionId 需匹配 `[a-zA-Z0-9_-]+`，非法格式返回 400 `INVALID_SESSION_ID`

### 3.2 端点清单

| 方法 | 路径 | 请求体 | 响应 | 错误 |
|------|------|--------|------|------|
| POST | `/session/{id}/message` | `string` (纯文本) | SSE 流 | 空 body → 400; 非法 id → 400 |
| POST | `/model/switch` | `{"modelId":"gpt-4o"}` | `{"status":"ok"}` | 未知 model → 404 |
| POST | `/provider/switch` | `{"provider":"deepseek"}` | `{"status":"ok"}` | 未配置 provider → 404 |
| POST | `/thinking` | `{"enabled":true,"level":"medium"}` | `{"status":"ok"}` | 无效 level → 400 (level: minimal/low/medium/high/xhigh) |
| GET | `/models[?provider=X]` | — | `[{"id":"...","ownedBy":"..."}]` | — |
| GET | `/health` | — | `{"status":"ok","model":"...","provider":"..."}` | — |
| GET | `/sessions` | — | `["default","project-x"]` | — |
| POST | `/session/{id}/create` | — | `{"status":"created"}` | 已存在 → 409; 非法 id → 400 |
| POST | `/session/{id}/delete` | — | `{"status":"deleted"}` | 不存在 → 404 |
| GET | `/session/{id}/messages` | — | `[Message, ...]` | 不存在 → 404 |
| POST | `/session/{id}/save` | — | `{"status":"saved"}` | homeDir 未设 → 400 |
...
public sealed class AgentHttpClient : IAsyncDisposable
{
    ...
    // 模型 — 返回 bool 表示操作是否成功
    public Task<bool> SwitchModelAsync(string modelId, CancellationToken ct = default);
    public Task<bool> SwitchProviderAsync(string provider, CancellationToken ct = default);
