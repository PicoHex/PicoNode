...
app.MapGet("/", (WebContext _, CancellationToken _) =>
    ValueTask.FromResult(WebResults.Text(200, "hello from PicoNode.Http", "OK")));

app.MapGet("/info", (WebContext ctx, CancellationToken _) =>
{
    var protocol = ctx.Request.Version == PicoNode.Http.HttpVersion.Http11 ? "HTTP/1.1" : "HTTP/2";
    var body = $$"""{"protocol":"{{protocol}}","path":"{{ctx.Path}}","method":"{{ctx.Request.Method}}"}""";
    return ValueTask.FromResult(WebResults.Json(200, body, "OK"));
});

app.MapPost("/stream", async (WebContext ctx, CancellationToken ct) =>
{
...
app.MapPost("/echo", (WebContext ctx, CancellationToken _) =>
{
...
// SSE
var sseHandler = SseEndpoint.Create(async (sse, ct) =>
{
...
// WebSocket
app.MapGet("/ws", (WebContext ctx, CancellationToken _) =>
{
