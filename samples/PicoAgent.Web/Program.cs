...
// Static files (registered first → executed last, fallback for non-API)
var wwwroot = Path.Combine(AppContext.BaseDirectory, "wwwroot");
if (Directory.Exists(wwwroot))
    app.Use(new StaticFileMiddleware(wwwroot).InvokeAsync);

// Proxy /api/* → backend (registered last → executed first)
...
app.Use(
    async (ctx, next, ct) =>
    {
        ...
        if (ctx.Request.BodyStream.Length > 0)
        {
            ...
            await ctx.Request.BodyStream.CopyToAsync(ms, ct);
            ms.Position = 0;
        ...
        return new HttpResponse
        {
            ...
            [
                new(
                    "Content-Type",
                    resp.Content.Headers.ContentType?.MediaType ?? "application/json; charset=utf-8"
                ),
            ],
        };
    }
